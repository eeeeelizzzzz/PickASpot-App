...
Use this guide to create **your own** OK Eats spreadsheet, or to copy the maintainer's template if a link is provided in the root [README](../../README.md).
...
3. Paste the full contents of [`OkEatsDriveTimes.gs`](OkEatsDriveTimes.gs) from this repo.
4. **Save** (disk icon). Name the project e.g. `OK Eats`.
5. Reload the spreadsheet. You should see menu **OK Eats**.

### First-time authorization

- Run **OK Eats → Set up Config sheet**.
...
1. **OK Eats → Set up Config sheet** (once)
2. Edit **Config!B1** with your home address
3. Add restaurants (or keep example rows while testing)
4. **OK Eats → Look up missing addresses**
5. **OK Eats → Update all drive times**
...
Write-back lets the OK Eats app push changes **to** your sheet (new restaurants, visit dates, ratings, notes). Sync only pulls **from** the sheet; write-back requires a separate Apps Script deployment.

### Prerequisites

1. **OK Eats → Set up Config sheet** (creates the `Config` tab if needed).
2. Note the value in **Config!B2** — this is your write secret. The app sends it with every save request.
3. You can also open **OK Eats → Show write-back setup** in the spreadsheet for a short in-sheet reminder.
...
4. Set:
   - **Description** — e.g. `OK Eats write-back` (optional, for your reference)
...
> **Who has access: Anyone** is required so the browser-based OK Eats app can call the endpoint. The write secret in **Config!B2** prevents unauthorized edits.

### Connect write-back in the app

1. Open OK Eats → **My List** → **Sheet write-back**.
...
If you change `OkEatsDriveTimes.gs`, create a **New deployment** (or **Manage deployments → Edit → Version: New version → Deploy**) so the Web app URL serves the latest code. The URL usually stays the same when you update an existing deployment.
...
In the deployed OK Eats app (**My List** tab):
...
You do **not** need to fork or deploy if you only want a different restaurant list. See the root [README](../../README.md#use-the-demo-app-with-your-sheet) for the short version; the steps are the same as **Option A** or **B** above, then connect in **My List** on any OK Eats deployment (e.g. the [official app](https://eeeeelizzzzz.github.io/Pick_a_Spot/)).

Your data stays in your Google Sheet; the app is just a client.

## Fork and deploy your own app

See [Run your own OK Eats](../../README.md#run-your-own-ok-eats) in the root README.
...
| File | Purpose |
|------|---------|
| [`OkEatsDriveTimes.gs`](OkEatsDriveTimes.gs) | Apps Script: drive times, address lookup, sheet write API |
